 ...
 *   3. Calls bt_serial_init(queue) to start the BLE GATT server
 *   4. Launches schema_task on the queue via xTaskCreate
 *   5. Calls telemetry_init() to start the 1 Hz telemetry timer
 *
 * Related:
...
void app_main(void) {
    ...
    // 4. JSON command dispatch task — consumes the queue.
    ESP_LOGI(TAG, ">>> xTaskCreate schema");
    xTaskCreate(schema_task, "schema", 4096, (void *)msg_queue, 10, NULL);

    // 5. Telemetry — uses the BLE write API on first tick, so start it last.
